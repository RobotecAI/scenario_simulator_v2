...
void Raycaster::setDirection(const simulation_api_schema::LidarConfiguration & configuration, double
  ...
  std::vector<double> vertical_angles;
    ...
    const auto horizontal_resolution = configuration.horizontal_resolution();
  ...
  std::vector<geometry_msgs::msg::Quaternion> & directions, double max_distance, double min_distance)
{
  ...
  for (unsigned int i = 0; i < threads.size(); ++i) {
    ...
    threads[i] = std::thread(
      ...
      std::ref(thread_detected_ids[i]), std::ref(directions), max_distance, min_distance, std::ref(rotation_matrices_));
  }
  for (unsigned int i = 0; i < threads.size(); ++i) {
    threads[i].join();
