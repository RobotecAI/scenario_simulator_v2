...
public:
  ...
  void setDirection(const simulation_api_schema::LidarConfiguration & configuration, double horizontal_angle_start = 0,
...
private:
    ...
    std::reference_wrapper<const std::vector<Eigen::Matrix3d>> ref_rotation_matrices)
  {
    ...
    for (int i = thread_id; i < directions.size(); i += thread_count) {
      ...
      rayhit.ray.dir_y = rotation_mat(3);
      rayhit.ray.dir_z = rotation_mat(6);
      ...
      if (rayhit.hit.geomID != RTC_INVALID_GEOMETRY_ID) {
        ...
        pcl::PointXYZI p;
        {
          ...
          p.y = rotation_matrices.at(i)(3) * distance;
          p.z = rotation_matrices.at(i)(6) * distance;
        }
