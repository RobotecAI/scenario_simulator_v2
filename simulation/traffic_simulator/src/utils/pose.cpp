  ...
  -> LaneletDistance
{
  ...
  if (
    const auto longitudinal_distance = longitudinalDistance(
      ...
      hdmap_utils_ptr)) {
    distance.longitudinal = longitudinal_distance.value();
  }
  if (const auto lateral_distance = lateralDistance(from, to, allow_lane_change, hdmap_utils_ptr)) {
    distance.lateral = lateral_distance.value();
  }
  return distance;
}
  ...
  const std::shared_ptr<hdmap_utils::HdMapUtils> & hdmap_utils_ptr) -> LaneletDistance
{
  ...
  if (
    ...
    distance.longitudinal = longitudinal_bounding_box_distance.value();
  }
  if (
    ...
    distance.lateral = lateral_bounding_box_distance.value();
  }
  return distance;
}
