...
geometry_msgs::msg::Point PolylineTrajectoryFollower::getTargetPosition()
{
  /*
    ...
    */
  const auto target_position = trajectory_parameter_m->shape.vertices.front().position.position;
  ...
  const geometry_msgs::msg::Point target_position, const geometry_msgs::msg::Point position)
{
  const auto [distance_to_front_waypoint, remaining_time_to_front_waypoint] = std::make_tuple(
    ...
    (trajectory_parameter_m->base_time ? *trajectory_parameter_m->base_time : 0.0) +
      trajectory_parameter_m->shape.vertices.front().time - entity_status_m.time);
  /*
  ...
  double distance_to_front_waypoint)
{
  const auto [distance, remaining_time] = [&]() {
    if (const auto first_waypoint_with_arrival_time_specified = std::find_if(
          std::begin(trajectory_parameter_m->shape.vertices),
          std::end(trajectory_parameter_m->shape.vertices),
      ...
      auto total_distance_to = [&](auto last) {
        ...
        for (auto iter = std::begin(trajectory_parameter_m->shape.vertices);
      ...
      if (const auto remaining_time =
        ...
        throw common::Error(
          ...
          (trajectory_parameter_m->base_time ? "absolute" : "relative"),
...
void PolylineTrajectoryFollower::discardTheFrontWaypoint()
{
  ...
  if (
    trajectory_parameter_m->base_time and
    not std::isnan(trajectory_parameter_m->shape.vertices.front().time)) {
    trajectory_parameter_m->base_time = entity_status_m.time;
  }

  if (std::rotate(
        std::begin(trajectory_parameter_m->shape.vertices),
        std::begin(trajectory_parameter_m->shape.vertices) + 1,
        std::end(trajectory_parameter_m->shape.vertices));
      not trajectory_parameter_m->closed) {
